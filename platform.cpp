...
platform::platform(identity type, int x)
    :plat_type(type), position_x(x), position_y(500), broken_state(false), needs_erased(0), jumped(false), render(true), collide(true), frame(0)
    {
        ...
        else if(plat_type == moving_x || floating_bug && position_x < 200){
            x_vel = 1;
        }

        if (plat_type == moving_broken || floating_bug && position_x > 200){
            x_vel = -1;
        }
...
platform::platform(identity type, int x, int y)
    :plat_type(type), position_x(x), position_y(y), broken_state(false), width(60), height(20), needs_erased(0), jumped(false), render(true), collide(true), frame(0)
    {
        if (plat_type == normal || plat_type == broken || plat_type == moving_x || plat_type == moving_y || moving_broken){
            width = 98;
            height = 15;
...
int platform::platform_update(){
    ...
    //need a general one for when screen is moving? or make a new function
    if (plat_type == moving_x || plat_type == moving_broken || plat_type == floating_bug){
...
int platform::specialLogic(int collision, std::vector<platform>& plats){
    if (needs_erased == 1){
        return 1;
    }
    ...
    if (collision == 2){
        ...
        for (int i = 0; i < size; i++){
            if (plats[i].get_identity() == broken){
                plats[i].changeToGood();
            }
        }
        needs_erased = 1;
        return 0;
